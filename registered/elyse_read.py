...
def parse_elyse2(file, input_loc, output_loc, export = False, output_file_name ='elyse2_headsigns.txt'):
    # Reads in pdf
    """
    ...
    for p in pdf.pages:
        ...
        extracted = [get_delims(x) for x in extracted           # Split text into rows from get_delims function
                                                                # EXCLUDE
                if x.split(' ')[0] not in ['File:', 'Led']     #   Each page has extra two lines with file info
                and len(x.split(' '))!=1                       #   Remove page number
                and not x.startswith('Entry Route')]     #   Remove column headers
        to_df += extracted
    ...
    if export:
        ...
        elyse2.to_csv(output_file_name, index = False, sep = '\t')
